...
(defun get-eaglesoft-surface-iri (patient-id
				  surface-type-iri tooth-name instance-count)
  "Returns an iri for the tooth surface that was involved in a restoration procedure.  The iri is generated by the patient id, the type of surface, the name of the type of tooth, and a count variable that used differientiate surface intances that have the same patient-id/surface-type/tooth-name but are numerically distinct."
  (let ((uri nil))
    (setf uri 
	  (get-unique-individual-iri patient-id 
				     :salt *eaglesoft-salt*
