...
;; When get-eaglesoft-onlays-ont is ran, the program verifies that the action_codes and
...
;; recreated, the call get-eaglesoft-onlays-ont with :force-create-table key set to t.

(defun get-eaglesoft-onlays-ont (&key patient-id tooth limit-rows force-create-table)
  "Returns an ontology of the onlays contained in the Eaglesoft database. The patient-id key creates an ontology based on that specific patient. The tooth key is used to limit results to a specific tooth, and can be used in combination with the patient-id. The limit-rows key restricts the number of records returned from the database.  It is primarily used for testing. The force-create-table key is used to force the program to recreate the actions_codes and patient_history tables."
  ...
  (let ((results nil)
    ...
    (setf query (get-eaglesoft-onlays-query 
		 :patient-id patient-id :tooth tooth :limit-rows limit-rows))

    (with-ontology ont (:collecting t
			:base *eaglesoft-individual-onlays-iri-base*
			:ontology-iri  *eaglesoft-onlays-ontology-iri*)
	(;; import needed ontologies
	 ...
	 (with-eaglesoft (results query)
		(loop while (#"next" results) do
		     ...
		     ;; get axioms
		     (as (get-eaglesoft-onlays-axioms 
  ...
  (let ((axioms nil)
    ...
    (loop for tooth in teeth-list do
	 ...
	 (push `(annotation-assertion !rdfs:label 
         ...
         ;; declare instance of !ohd:'tooth to undergo onlay procedure role'
	 (setf tooth-role-uri (get-eaglesoft-tooth-to-undergo-onlay-procedure-role-iri
	 ...
	 (setf restoration-name (get-eaglesoft-onlay-restoration-name ada-code))
	 (push `(annotation-assertion !rdfs:label 
				      ,restoration-uri
	 ...
	 (push `(annotation-assertion !rdfs:label
				      ...
				      ,(str+ "billing code " ada-code " for " restoration-name 
					     " onlay restoration on " tooth-name 
					     " of patient " patient-id)) axioms)
...
(defun get-eaglesoft-tooth-to-undergo-onlay-procedure-role-iri (patient-id tooth-name instance-count)
  "Returns an iri for a 'tooth to undergo onlay procedure role' that is generated by the patient id, the name of the type of the tooth, and a count variable that used differientiate tooth role intances that have the same patient-id/tooth-name but are numerically distinct."
  (let ((uri nil))
    (setf uri 
	  (get-unique-individual-iri patient-id 
				     :salt *eaglesoft-salt*
				     :iri-base *eaglesoft-individual-teeth-iri-base*
				     :class-type !'tooth to undergo onlay procedure role'@ohd
				     :args `(,tooth-name ,instance-count "eaglesoft")))
    ;; return uri
    uri))

(defun get-eaglesoft-onlay-restoration-iri (patient-id tooth-name
  ...
  (let ((uri nil))
    (setf uri 
	  (get-unique-individual-iri patient-id 
				     :salt *eaglesoft-salt*
				     :iri-base *eaglesoft-individual-onlays-iri-base*
				     :class-type restoration-type-iri
				     :args `(,tooth-name ,instance-count "eaglesoft")))
    ;; return uri
    uri))

(defun get-eaglesoft-onlay-material-iri (patient-id tooth-name
...
(defun get-eaglesoft-onlay-restoration-name (ada-code)
...
(defun get-eaglesoft-onlays-query (&key patient-id tooth limit-rows)
...
#| 
This queries the eaglesoft database for all ADA codes that indicate an onlay procedure
has been performed.
