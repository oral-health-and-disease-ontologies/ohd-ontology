...
owlim_se_r21_remote <- "http://den287.sdm.buffalo.edu:8080/openrdf-workbench/repositories/ohd-r21-nightly/query"
...
patients_query_count <- paste(prefixes, patients_query_count)

patient_get_count_query <- 
"PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT (count(distinct ?patientid) as ?count)
WHERE { 
...
ORDER BY ?patientid
LIMIT 10 "
patient_list_query <- paste(prefixes, patient_list_query)

r21_query_by_patient <-
"PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX obo: <http://purl.obolibrary.org/obo/>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
...
#materials_query <- paste(prefixes, materials_query)
...
#unerupted_query <- paste(prefixes, unerupted_query)
...
#caries_query <- paste(prefixes, caries_query)
...
#missing_query <- paste(prefixes, missing_query)
