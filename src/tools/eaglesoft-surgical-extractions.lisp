...
(defun get-eaglesoft-tooth-to-be-extracted-role-iri (patient-id tooth-name instance-count)
  "Returns an iri for a 'tooth to be extracted role' identified in the Eaglesoft database by the patient id, the name of the type of the tooth, and a count variable that used differientiate tooth role intances that have the same patient-id/tooth-name but are numerically distinct."
  (let ((uri nil))
    (setf uri 
