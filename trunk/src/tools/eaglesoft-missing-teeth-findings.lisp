...
(defun get-eaglesoft-missing-tooth-finding-axioms 
    (patient-id occurrence-date tooth-data)
  (let ((axioms nil)
	(patient-uri nil)
	(finding-uri nil)
    ...
    (loop for tooth in teeth-list do
	 ...
	 (setf finding-uri
	       (get-eaglesoft-missing-tooth-finding-iri patient-id tooth))
...
(defun get-eaglesoft-missing-tooth-finding-iri (patient-id tooth-name)
  "Returns an iri for a 'missing tooth finding' that is generated by the patient id, and the name of the type of the tooth."
  (let ((uri nil))
    (setf uri 
