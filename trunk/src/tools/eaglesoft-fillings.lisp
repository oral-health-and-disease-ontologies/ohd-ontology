...
(defun get-eaglesoft-tooth-to-be-filled-role-iri (patient-id tooth-name instance-count)
  "Returns an iri for a 'tooth to be filled role' identified in the Eaglesoft database by the patient id, the name of the type of the tooth, and a count variable that used differientiate tooth role intances that have the same patient-id/tooth-name but are numerically distinct."
  (let ((uri nil))
    (setf uri 
...
(defun get-eaglesoft-filling-restoration-iri (patient-id tooth-name
  ...
  "Returns an iri for a restoration filling procedure identified in the Eaglesoft database by the patient id, the name of the type of tooth, the type of restoration's iri, and a count variable that used differientiate restoration procedure intances that have the same patient-id/restoration-type-iri but are numerically distinct."
  (let ((uri nil))
    (setf uri 
	  (get-unique-individual-iri patient-id 
				     :salt *eaglesoft-salt*
...
(defun get-eaglesoft-filling-material-iri (patient-id tooth-name
  ...
  "Returns an iri for the material used in restoration filling procedure identified in the Eaglesoft database by the patient id, the name of the type of tooth, the type of restoration's material, and a count variable that used differientiate restoration procedure intances that have the same patient-id/material-type-iri but are numerically distinct."
  (let ((uri nil))
    (setf uri 
	  (get-unique-individual-iri patient-id 
				     :salt *eaglesoft-salt*
